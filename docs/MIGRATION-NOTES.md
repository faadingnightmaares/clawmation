...
**Reading the source comments.** Comments throughout `src/` and `src-tauri/src/`
cite the Python original by its own filenames: `ui_app.py`, `recorder.py`,
`capture.py`, `index.html`, `anime_macro/config.py`, and the `clawmation_vision`
sidecar package. Those files are not in this repository; the citations are
provenance, explaining *why* a constant or an ordering is what it is. Nothing in
the build depends on them.
...
- **ML-OCR (EasyOCR / torch)**: lazy-imported and excluded from the Python
  production build, so every `method = "ocr"` guard failed on import there. The
  port replaces it with the platform engine (`Windows.Media.Ocr`, see
  `hardware/ocr.rs`): no model download, no torch, and text guards actually work.
  With no OCR language pack installed the method reports "unavailable" and points
  at Settings › Time & language, which is the only remaining dead-end.
...
- **Two structural facts of low-level hooks are handled deliberately.** (1) The
  ...
  procs lock. That is honest, since a system-wide hook is inherently a singleton
  (as are pynput's module-level listeners). (2) A low-level hook only fires while
  its installing thread pumps messages, so recording runs on a dedicated thread
  with a `GetMessage` loop, stopped by `PostThreadMessage(WM_QUIT)`.
...
- **`key_name` is the exact inverse of `input.rs::resolve_vk`** (verified by the
  ...
  **unshifted** base character (`A`→`"a"`); that is replay-equivalent to pynput
  ...
  `scroll_lock`, `f13` to `f20`) record faithfully but replay as no-ops, exactly
  as in Python. None occur in this app's macros (0 of 604 real events).
...
- **Two Python conveniences are dropped as dead code.** `_densify_moves` (the
  move-interpolation helper) has no caller, and the play loop's own comment says
...
- **The `windows` crate is used here (and only here) instead of `windows-sys`.**
  ...
  `xcap`, was rejected: `xcap` returns an `image::RgbaImage` (which would have
  pre-settled the then-open vision-stack decision) and its recorder model
  doesn't match dxcam's reuse-on-timeout semantics, so we'd end up
...
- **Capture opens lazily, on first detection.** `core::Vision` constructs it on
  the first `grab()` and `commands/status.rs` reports the *resolved* backend from
  then on (`config.capture_backend` with `0.0` fps until then, matching Python's
  `self._capture.backend if self._capture else self.config.capture_backend`). The
  COM handles are `Send`, so the guard, vision, and preview threads all share the
  one capture behind `core::Vision`'s mutex.
